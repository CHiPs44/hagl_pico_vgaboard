...
uint8_t *framebuffer0 = (uint8_t *)_framebuffer0;
// Poor man's alignment to 32 bits...
// uint32_t PICO_VGABOARD_DATA _framebuffer1[(FB_WIDTH * FB_HEIGHT / 2) / 4];
uint8_t *framebuffer1 = NULL; //(uint8_t *)_framebuffer1;
...
void main(void)
{
    ...
    pico_vgaboard_framebuffer_init(
        framebuffer, 0, framebuffer0, framebuffer1, FB_DOUBLE_BUFFER, FB_DEPTH, (uint16_t *)palette_4bpp_ansi,
    ...
    {
        pico_vgaboard_framebuffer_put_pixel(
            framebuffer,
            rand() % framebuffer->window_width, rand() % framebuffer->window_height,
            i % 16);
    }
